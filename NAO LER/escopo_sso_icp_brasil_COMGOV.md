...
Este documento apresenta o escopo completo de um sistema de autenticação centralizada (SSO) cujo primeiro objetivo é permitir **login universal com certificado digital ICP-Brasil**, aceitando certificados válidos emitidos por qualquer Autoridade Certificadora credenciada na cadeia da ICP-Brasil, e cujo segundo objetivo, em etapa posterior, é incorporar **login federado via gov.br**.

A base normativa da ICP-Brasil prevê que aplicações que admitam determinado tipo de certificado contemplado pela ICP-Brasil **devem aceitar qualquer certificado de mesmo tipo, ou superior, emitido por qualquer AC credenciada pela AC Raiz**. Além disso, os certificados de tipo A são adequados para confirmação de identidade e assinatura de documentos eletrônicos. citeturn147318view0turn147318view3

Na prática, isso significa que o sistema não deve ficar amarrado a um único fornecedor. O usuário poderá autenticar-se com certificados emitidos por provedores como Serpro, Valid, Soluti, Certisign e outros integrantes da cadeia ICP-Brasil, desde que o certificado seja tecnicamente válido, esteja dentro do prazo, não esteja revogado e pertença ao titular correto. citeturn147318view0turn147318view3
...
- evoluir, em segunda fase, para um modelo híbrido com **ICP-Brasil + gov.br**.
...
1. **Agnóstico de fornecedor**: aceitar qualquer certificado ICP-Brasil compatível, e não apenas SerproID.
2. **Aderência à cadeia ICP-Brasil**: validar a cadeia até a AC Raiz da ICP-Brasil, operada no âmbito do ITI. citeturn147318view0turn207914search7
...
7. **Expansibilidade**: arquitetura preparada para incluir gov.br na fase seguinte.
...
A exigência de aceitar qualquer certificado de mesmo tipo, ou superior, emitido por qualquer AC credenciada pela AC Raiz, decorre dos requisitos mínimos das políticas de certificado da ICP-Brasil. citeturn147318view0turn147318view3

#### 4.2.2. Cadastro e vinculação de identidade

O sistema deve possuir base própria de usuários e permitir o vínculo entre a identidade do certificado e a conta institucional. Exemplos de estratégias de vinculação:
...
No caso de integração com SerproID, a documentação pública do SerproID mostra endpoints para recuperação do certificado autorizado, homologação de integração e operações baseadas em token, o que confirma a viabilidade de um fluxo de assinatura remota em arquitetura separada do login universal ICP-Brasil. citeturn147318view1turn147318view2
...
Deve-se checar se o tipo e o uso do certificado são compatíveis com a autenticação pretendida. A documentação da ICP-Brasil estabelece que certificados do tipo A são utilizados em aplicações como confirmação de identidade e assinatura de documentos eletrônicos. citeturn147318view0turn147318view3
...
O sistema deve decidir se:

- permite autoassociação do primeiro certificado;
- exige pré-cadastro do usuário;
- exige aprovação administrativa;
- permite múltiplos certificados por usuário;
- trata certificados PJ e PF com regras diferentes.
...
Esse ponto é importante: o **login universal ICP-Brasil** é o objetivo de interoperabilidade; já certificados em nuvem, como os do SerproID, podem exigir **integrações próprias**, embora continuem ancorados na lógica de identidade baseada em certificado. A própria documentação do SerproID mostra ambiente de homologação e endpoints específicos de integração. citeturn147318view1turn147318view2
...
### 8.1. Usuário final
Pessoa física ou representante de pessoa jurídica que realiza autenticação com certificado digital.
...
- proteção contra replay, CSRF e fixation;
- hardening de sessão;
...
4. Se não houver vínculo automático permitido, abre fluxo de aprovação.
...
5. Não autenticar usuário cujo CPF/CNPJ não esteja vinculado à conta permitida.
6. Permitir múltiplos certificados por usuário, com rastreabilidade.
7. Exigir aprovação para casos excepcionais.
8. Bloquear automaticamente padrões suspeitos de autenticação.
...
## 17. Fase 2 — evolução para gov.br

Depois da entrega do núcleo ICP-Brasil, o sistema poderá incorporar **gov.br** como segundo fator de expansão funcional, criando um SSO híbrido.

### 17.1. Objetivo da fase 2
Permitir que sistemas clientes escolham políticas como:

- login somente com ICP-Brasil;
- login somente com gov.br;
- login por qualquer um dos dois;
- login com ICP-Brasil para perfis críticos e gov.br para perfis comuns.

### 17.2. Benefícios da fase 2
- ampliação do público atendido;
- redução de barreira de acesso;
- maior flexibilidade de identidade digital.

### 17.3. Pré-requisito
A base de identidade, vínculo e federação da fase ICP-Brasil deve estar sólida antes da entrada do gov.br.
...
- suporte irrestrito a todos os certificados remotos sem integração dedicada.
...
É plenamente viável construir um sistema que funcione como **SSO baseado em ICP-Brasil**, desde que a implementação trate corretamente a validação da cadeia até a AC Raiz, a validade temporal, a revogação, os atributos do certificado, o vínculo com a identidade interna e a trilha de auditoria. A base normativa da ICP-Brasil dá sustentação à interoperabilidade entre certificados do mesmo tipo, ou superior, emitidos por quaisquer ACs credenciadas na cadeia. citeturn147318view0turn147318view3

A recomendação estratégica é começar pelo **login universal ICP-Brasil**, consolidando o núcleo de identidade, sessão, auditoria e integração com sistemas clientes. Depois, em segunda etapa, evoluir para um modelo híbrido com **gov.br**, preservando a interoperabilidade, a segurança e a governança institucional.
